...
class MARDscraper(BaseScraper):
    SOURCE_NAME = "MARD"

    def parse_article(self, url: str, page: Page) -> Dict[str, Any]:
        soup = self.load_page(page, url)

        title = self.extract_text(soup, ["h1.title", "h1.entry-title", "h1"])
        date_text = self.extract_text(soup, ["span.date", "time", "div.post-date"])
        ...
        return {
            ...
            "tags": ["dữ_liệu_cào", "nông_nghiệp", "mard"],
        }
